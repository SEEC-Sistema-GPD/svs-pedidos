...
function CustomExpression($value, $data, $field, $ptype, $table="")
{
	...
	if(!$table)
		$table = $strTableName;
				if($table=="np" && $field=="saldo1")
	{
		
// Definir a cor com base no valor
if ($value < 0) {  
    $color = "red";  // Vermelho para valores negativos  
} else {  
    $color = "black";  // Preto para valores positivos ou zero  
}

// Formatar o valor como moeda (R$) em português
$valueFormatted = "R$ " . number_format($value, 2, ',', '.');

// Aplicar a cor e a formatação
$value = "<span style='color: " . $color . "'>" . $valueFormatted . "</span>";

;
		return $value;
	}
				if($table=="pedidos" && $field=="valor - debito")
	{
		if ($value < 0) {  
    $color = "red";  // Vermelho para valores negativos  
} else {  
...
;
		return $value;
				...
				if($table=="NP_escola" && $field=="saldo1")
	{
...
;
		return $value;
				...
				if($table=="Pedidos_escola" && $field=="valor - debito")
	{
		
if ($value < 0) {  
    $color = "red";  // Vermelho para valores negativos  
  {
    // Alerta de alta urgência
    echo '<script type="text/javascript">
            alert("🚨🚨🚨 ALERTA! Você ultrapassou o saldo disponível. Ajuste o pedido! 💥");
          </script>';
}
...
{
	$value = "";
				if($table=="arquivos" && $field=="Arquivo")
	{
		;
		return $value;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="np" && $field=="Data")
	{
		return now();
	}
				if($table=="np" && $field=="Valor")
	{
		return 0;
	}
				if($table=="np" && $field=="Saldo")
	{
		return 0;
	}
				if($table=="np" && $field=="Execício")
	{
		return date('Y');
	}
				if($table=="np" && $field=="Debito")
	{
		return 0;
	}
				if($table=="np" && $field=="saldo1")
	{
		return 0;
				...
				if($table=="np" && $field=="Sd_ant")
	{
		return 0;
	}
				if($table=="pedidos" && $field=="Exercicio")
	{
		return date('Y');
	}
				if($table=="pedidos" && $field=="status")
	{
		return "Pendente";
	}
				if($table=="pedidos" && $field=="Data_cad")
	{
		return now();
	}
				if($table=="pedidos_view" && $field=="status")
	{
		return "Pendente";
	}
				if($table=="NP_escola" && $field=="Data")
	{
		return now();
	}
				if($table=="NP_escola" && $field=="Valor")
	{
		return 0;
	}
				if($table=="NP_escola" && $field=="Execício")
	{
		return date('Y');
	}
				if($table=="NP_escola" && $field=="Debito")
	{
		return 0;
	}
				if($table=="NP_escola" && $field=="saldo1")
	{
		return 0;
				...
				if($table=="NP_escola" && $field=="Sd_ant")
	{
		return 0;
	}
				if($table=="Pedidos_escola" && $field=="Exercicio")
	{
		return date('Y');
	}
				if($table=="Pedidos_escola" && $field=="status")
	{
		return "Pendente";
	}
				if($table=="Pedidos_escola" && $field=="Data_cad")
	{
		return now();
	}
	return "";
