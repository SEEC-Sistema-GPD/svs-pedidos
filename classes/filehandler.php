...
class RunnerFileHandler {
	...
	protected function validateFile( $filename, $fileSize, &$errorMessage ) {
        if ( !$filename ) {
            $errorMessage = "O nome do arquivo não foi fornecido";
            return false;
        }
		...
		if( $acceptedTypes ) {
			...
			if( array_search( $ext, $acceptedTypes ) === false && array_search( ".".$ext, $acceptedTypes ) === false ) {
				$errorMessage = "O tipo de arquivo não é permitido";
				return false;
			}
		}
		...
		if ( $maxFileSize && $fileSize > $maxFileSize * 1024 ) {
            $errorMessage = mysprintf("O tamanho do arquivo excede o limite de %s kbytes", array( $maxFileSize ) );
            return false;
        }
		...
		if ( $maxTotalFileSize ) {
			if( $this->getUploadFilesSize() + $fileSize > $maxTotalFileSize * 1024 ) {
				$errorMessage = mysprintf( "O tamanho total dos arquivos excede o limite de %s kbytes", array( $maxTotalFileSize ) );
				return false;
			}
		}
		...
		if ( $maxNumberOfFiles && $this->getUploadFilesCount() >= $maxNumberOfFiles ) {
			$errorMessage = mysprintf( "Não poderá ser feito o upload de mais de %s arquivos", array( $maxNumberOfFiles ) );
			return false;
		}
        return true;
	}
