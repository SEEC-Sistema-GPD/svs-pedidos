...
class Security
{
	...
	public static function getUserGroups()
	{
		...
		$sql = "select ". $grConnection->addFieldWrappers( "Label" )
			." from ". $grConnection->addTableWrappers( "pedidosuggroups" ) . " WHERE " . $grConnection->addFieldWrappers( "GroupID" )
	...
	*  For AD it is <provider code><username>
	...
	public static function getUgMembersDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "pedidosugmembers", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgGroupsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "pedidosuggroups", $cman->getUserGroupsConnId() );
	}
	...
	public static function getUgRightsDatasource() {
		...
		global $cman;
		return getDbTableDataSource( "pedidosugrights", $cman->getUserGroupsConnId() );
	}
	...
	public static function  fillTablesOwnerId( $data, $provider ) {
		...
		if( $securityType == stAD && !$provider["useDbGroups"] ) {
			...
			storageSet( "_np_OwnerID", $userId );
						storageSet( "_escolas_OwnerID", $userId );
						storageSet( "_pedidos_view_OwnerID", $userId );
						storageSet( "_fornecedores_view_OwnerID", $userId );
						storageSet( "_NP_escola_OwnerID", $userId );
						storageSet( "_Pedidos_escola_OwnerID", $userId );
						storageSet( "_Sd_ant_OwnerID", $userId );
		} else {
						storageSet( "OwnerID", $data["username"] );
			storageSet( "_np_OwnerID", $data["username"] );
						storageSet( "_escolas_OwnerID", $data["username"] );
						storageSet( "_pedidos_view_OwnerID", $data["username"] );
						storageSet( "_fornecedores_view_OwnerID", $data["username"] );
						storageSet( "_NP_escola_OwnerID", $data["username"] );
						storageSet( "_Pedidos_escola_OwnerID", $data["username"] );
						storageSet( "_Sd_ant_OwnerID", $data["username"] );
		}
	}
